# Docker nginx + php7.4 container

Docker container based on `Ubuntu 16.04` version. Using `nginx stable` version with `php 7.4`. This is simple container for my own personally purpose. If you have an idea how to improve it, raise a GitHub issue.
 ...
 * git, composer
 * php 7.4 (fpm, cli, mysql, apc, curl, gd, intl, mcrypt, mbstring, memcache, memcached, sqlite, tidy, xmlrpc, xsl, pgsql,  ldap, sybase, odbc, soap, json, xdebug, imagick)
...
```yaml
  web:
    ...
    volumes:
      ...
      - /mnt/docker/php/custom.ini:/etc/php/7.4/fpm/conf.d/custom.ini
